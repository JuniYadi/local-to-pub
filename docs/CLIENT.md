...
export TUNNEL_LOCAL_REQUEST_TIMEOUT_MS="1200000"  # local fetch timeout ms (default 1200000, 20 min)
```

> **Note:** The server default `TUNNEL_REQUEST_TIMEOUT_MS` is `1205000`, 5s above the client default.
> If you override the client timeout, also set the server env var to a value at least 5s larger.
